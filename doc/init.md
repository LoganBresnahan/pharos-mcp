...
- **Pool does not auto-evict crashed LSPs.** When an LSP child process dies, the pool's cached Client struct still points at the dead Port; the next tool call surfaces a transport error to the LLM but the cache is not cleared. **Closed in M9 Phase B** — pool now monitors each `lsp_proc` and auto-evicts on DOWN.
- **gopls regression after M9 Phase B.** Re-dogfood post-`lsp_proc` migration: every gopls call returns `LSP transport error`, persistent across multiple attempts (no cold-cache recovery). rust-analyzer, tsserver, pyright all pass. Hypothesis: ownership transfer race during `proc.start` — gopls emits unsolicited messages (e.g. `window/workDoneProgress/create`, `workspace/configuration`) immediately after `initialize` that land in the wrong process mailbox, and `lifecycle.wait_for_response` inside the actor never sees them. Fix candidates: (a) drain ALL messages inside the actor before publishing the Subject, (b) use `actor.on_init`-equivalent so Port is owned by actor from creation (avoiding `client.connect` transfer entirely), (c) add a port monitor that auto-evicts the proc on persistent transport error rather than relying solely on actor death. Reproducer: hit gopls via `mcp__pharos__document_symbols` on a `.go` file after fresh pharos boot.
