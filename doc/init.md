...
- **`signature_help` returns `LSP transport error` on every server tested.** Stage 2 #1 (real ClientCapabilities) was expected to fix this and did not. Affects rust-analyzer and gopls in dogfood. Reproduces via `lsp_request_raw` with 30s timeout. Hypothesis: capability sub-fields not declared rich enough (rust-analyzer wants `signatureInformation.parameterInformation.labelOffsetSupport`, `activeParameterSupport`), or the `triggerKind`/`triggerCharacter` `context` field is required. Stage 2 follow-up: extend capabilities + try richer trigger context.
- **`format_document` works on gopls; still errors on rust-analyzer.** Stage 2 #1 fix worked for one server, not the other. rust-analyzer shells out to `rustfmt`; suspected per-tool timeout (10s) too short for the cold-cache shellout. Stage 2 follow-up: bump per-tool timeout for `format_document` specifically.
- **`code_actions` fixed by Stage 2 #1.** No longer in this list — left as a marker that capabilities advertisement was the right fix for at least one of the trio.
