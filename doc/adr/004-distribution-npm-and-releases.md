...
Meta package `pharos` declares per-platform sub-packages as `optionalDependencies`. Each sub-package targets exactly one platform via `os` + `cpu` fields and contains exactly one binary. npm only installs the matching sub-package; others are silently skipped (that's what `optional` means in this context). Meta package's `bin` is a tiny Node shim that resolves the matching sub-package and `spawn`s its binary. No postinstall, no network at install (binaries are inside the npm packages already). Robust, fast, used in production by major projects.
...
|---------|----------|--------------|
| `@pharos/linux-x64` | one binary | linux / x64 |
| `@pharos/linux-arm64` | one binary | linux / arm64 |
| `@pharos/darwin-x64` | one binary | darwin / x64 |
| `@pharos/darwin-arm64` | one binary | darwin / arm64 |
| `@pharos/win32-x64` | one binary (.exe) | win32 / x64 |
| `pharos` | Node shim + `optionalDependencies` of all five above pinned to exact version | (any) |

Meta package's `bin/pharos.js`:
```javascript
#!/usr/bin/env node
const { spawn } = require('child_process')
const pkg = `@pharos/${process.platform}-${process.arch}`
const ext = process.platform === 'win32' ? '.exe' : ''
const bin = require.resolve(`${pkg}/pharos${ext}`)
...
User config:
```json
{ "mcpServers": { "pharos": { "command": "npx", "args": ["-y", "pharos"] } } }
```
...
- npm scope `@pharos/*` must be claimed and configured. `NPM_TOKEN` must have publish permission on the scope.
...
- npm cold start: `npx -y pharos` first-time fetch is slower than `cargo install` style. Subsequent runs are fast (cached).
