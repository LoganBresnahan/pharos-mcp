...
The project is being developed in conversation with an instance of Claude running inside Claude Code. That instance can be configured as a real consumer of `pharos` while development is happening — restart the harness, the new tools register, the LLM uses them in the next turn. This is the most direct possible feedback loop: the same kind of consumer that will use the product in production validates each piece as it ships.
...
2. The developer adds (or already has) `pharos` configured in their Claude Code MCP config:
   ```json
   {
     "mcpServers": {
       "pharos": {
...
4. In the next conversation turn, the LLM has the tools registered as `mcp__pharos__<tool_name>`.
5. The developer asks the LLM to exercise the new tools against the project's own source code: "use `mcp__pharos__hover` on line 47 of `src/pharos/mcp/server.gleam` and tell me what it says", etc.
6. The LLM reports:
...
| 7 (extension bridge) | With `pharos_ext` running in VSCode, Claude reads diagnostics on an unsaved buffer and confirms it sees the unsaved version |
