//// MCP tool: `goto_type_definition`.
////
//// Wraps LSP `textDocument/typeDefinition`. Returns the type's
//// definition site(s) for the symbol at the given position — for
//// instance, in `let x: Foo = bar()`, calling this on `x` returns
//// the location where `Foo` is declared, not the location where
//// `x` is bound. Same response shape as `goto_definition`:
//// Location, list of Location, list of LocationLink, or null.
...
pub type GotoTypeDefinitionError {
...
pub fn handle(
  ...
  line: Int,
  character: Int,
) -> Result(String, GotoTypeDefinitionError) {
  let params =
    json.object([
  ...
  case
    session.with_session_and_retry(pool, file_uri, fn(lsp) {
      session.request_with_content_modified_retry(fn() {
        proc.request(lsp, "textDocument/typeDefinition", params, default_timeout_ms)
      })
    })
  {
